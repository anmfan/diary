...
import {useGetGroupDataQuery} from "@/redux/api/teacher-management-api.ts";
...
const TeacherManagement = () => {
    ...
    const { data, isLoading } = useGetGroupDataQuery({ email: email! })
            ...
            <div className={styles.content}>
                <div className={styles.listContainer}>
                    <div className={styles.listTitle}>
                        <h2>Студенты группы: <span>{data?.students_count}</span></h2>
                        <div className={styles.rightTitleBlock}>
                            <svg onClick={() => exportGroupStudentsToExcel(data?.students || null, data?.name || '')} xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                    ...
                    <TeacherManagementListStudents data={data}/>
                </div>
            </div>
