...
const ModalEditRemoveGroup = ({item, groups}: TModalEditRemoveGroup) => {
    const dispatch = useAppDispatch();
    const { closeModal } = useModal();
    const { setSelected } = useSetSelectedItem();
    const userId = item?.id || 0

    const handleClick = (group_name: string) => {
        dispatch(removeGroup({ user_id: userId, group_name }))
        closeModal()
        setSelected(null)
    }
    ...
    return (
        <div className={styles.btnRemoveBlock}>
            {groups?.map(group => (
                <div className={styles.btnRemoveLine} key={group}>
                    <span>{group}</span>
                    <svg onClick={() => handleClick(group)} aria-label="Открепить" xmlns="http://www.w3.org/2000/svg" width={18} height={18} fill="currentColor" className={styles.biTrash3}
                         viewBox="0 0 16 16">
                        <path
